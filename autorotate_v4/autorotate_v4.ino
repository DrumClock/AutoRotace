...
void loop() {
  ...
  if (pos != lastPos) {
    ...
    // Změna směru otáčení: invertujeme hodnotu pos (množství otáček)
    ledPos = (-pos) % NumPixels;  // Opačné znaménko

    if (ledPos < 0) {
      ledPos += NumPixels;  // Pokud je ledPos negativní, přičteme NumPixels
    }
  }
